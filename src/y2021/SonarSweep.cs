...
{	
	...
	public long Silver(IEnumerable<string> input) => CountIncreases(input.Select(x => Convert.ToInt32(x)));
	
	public long Gold(IEnumerable<string> input) => CountIncreases(input.Select(x => Convert.ToInt32(x)).Window(3).Select(window => window.Sum()));
}
